...
class TestUniformRandomParentSelection:
	def test_output_size(self):
		for _ in range(iterations):
			...
			assert len(sel.uniform_random_selection(pop, outsize, **config['parent_selection_kwargs'])) == outsize
	...
	def test_is_uniform(self):
		...
		for _ in range(iterations):
			...
			for individual in sel.uniform_random_selection(pop, outsize, **config['parent_selection_kwargs']):
				hits[individual] += 1
	...
	def test_population_unmodified(self):
		for _ in range(iterations):
			...
			selection = sel.uniform_random_selection(pop, random.randint(1, popsize * 2), **config['parent_selection_kwargs'])
			for i in range(popsize):
				assert same_object(pop[i], copies[i])

class TestKTournamentWithReplacement:
	def test_output_size(self):
		kwargs = copy.deepcopy(config['parent_selection_kwargs'])
		del kwargs['k']
		for _ in range(iterations):
			pop = random_pop(popsize, boardsize)
			random_fitness(pop)
			...
			assert len(sel.k_tournament_with_replacement(pop, outsize, **kwargs, k=k)) == outsize
		assert len(sel.k_tournament_with_replacement(pop, outsize, **kwargs, k=popsize)) == outsize
		assert len(sel.k_tournament_with_replacement(pop, outsize, **kwargs, k=2)) == outsize
	...
	def test_elitist(self):
		...
		for _ in range(iterations * 10): #this one is very prone to false passes
			...
			out = sel.k_tournament_with_replacement(pop, outsize, **kwargs, k=k)
	...
	def test_is_based_on_fitness(self):
		...
		for _ in range(iterations):
			...
			hits = {x:0 for x in pop}
			k = random.randint(2, popsize-1)
			for individual in sel.k_tournament_with_replacement(pop, outsize, **kwargs, k=k):
				hits[individual] += 1
	...
	def test_definitely_duplicates(self):
		...
		hits = {x:0 for x in pop}
		outsize = popsize + 1
		for individual in sel.k_tournament_with_replacement(pop, outsize, **kwargs, k=2):
			hits[individual] += 1
		ones = 0
		greater = 0
		for hit in hits.values():
	...
	def test_fails_on_impossible_k(self):
		...
		with pytest.raises(Exception):
			sel.k_tournament_with_replacement(pop, popsize, **kwargs, k=popsize+1)
	...
	def test_population_unmodified(self):
		...
		for _ in range(iterations):
			...
			random_fitness(pop)
			copies = [copy.deepcopy(x) for x in pop]
			selection = sel.k_tournament_with_replacement(pop, random.randint(1, popsize * 2), **kwargs, k=random.randint(2, popsize))
			for i in range(popsize):
				assert same_object(pop[i], copies[i])
...
class TestKTournamentWithoutReplacement:
	def test_output_size(self):
		...
		for _ in range(iterations):
			...
			assert len(sel.k_tournament_without_replacement(pop, outsize, **kwargs, k=k)) == outsize
	...
	def test_elitist(self):
		...
		for _ in range(iterations * 10): #this one is very prone to false passes
			...
			out = sel.k_tournament_without_replacement(pop, outsize, **kwargs, k=k)
			for ind in worst_individuals:
				assert ind not in out
			failed = True
	...
	def test_is_based_on_fitness(self):
		...
		out_of_order = 0
		for _ in range(iterations):
			for individual in sel.k_tournament_without_replacement(pop, outsize, **kwargs, k=k):
				hits[individual] += 1
		ordering = sorted(pop, key=lambda x:x.fitness)
		for i in range(len(ordering) - 1):
			if hits[ordering[i]] > hits[ordering[i + 1]]:
	...
	def test_definitely_no_duplicates(self):
		...
		for _ in range(iterations):
			...
			k = random.randint(2, popsize - outsize)
			hits = {x:0 for x in pop}
			for individual in sel.k_tournament_without_replacement(pop, outsize, **kwargs, k=2):
				hits[individual] += 1
			for hit in hits.values():
				assert hit <= 1
	...
	def test_fails_on_impossible_k(self):
		...
		k = (popsize - outsize) + 2
		with pytest.raises(Exception):
			sel.k_tournament_without_replacement(pop, outsize, **kwargs, k=k)
	...
	def test_population_unmodified(self):
		...
		for _ in range(iterations):
			...
			outsize = random.randint(1, popsize - 3)
			k = random.randint(2, popsize - outsize)
			selection = sel.k_tournament_without_replacement(pop, outsize, **kwargs, k=k)
			for i in range(popsize):
				assert same_object(pop[i], copies[i])
...
class TestFitnessProportionate:
	def test_output_size(self):
		for _ in range(iterations):
			...
			random_fitness(pop)
			outsize = random.randint(1, popsize * 2)
			assert len(sel.fitness_proportionate_selection(pop, outsize, **config['parent_selection_kwargs'])) == outsize
	...
	def test_strongly_prefers_outliers(self):
		...
		for _ in range(iterations):
			...
			not_it = 0
			for ind in sel.fitness_proportionate_selection(pop, outsize, **config['parent_selection_kwargs']):
	...
	def test_is_uniform(self):
		...
		for _ in range(iterations):
			...
			hits = {x:0 for x in pop}
			out = sel.fitness_proportionate_selection(pop, outsize, **config['parent_selection_kwargs'])
	...
	def test_definitely_duplicates(self):
		...
		hits = {x:0 for x in pop}
		outsize = popsize + 1
		for individual in sel.fitness_proportionate_selection(pop, outsize, **config['parent_selection_kwargs']):
			hits[individual] += 1
		ones = 0
		greater = 0
		for hit in hits.values():
	...
	def test_population_unmodified(self):
		for _ in range(iterations):
			...
			copies = [copy.deepcopy(x) for x in pop]
			outsize = random.randint(1, popsize * 2)
			selection = sel.fitness_proportionate_selection(pop, outsize, **config['parent_selection_kwargs'])
			for i in range(popsize):
				assert same_object(pop[i], copies[i])
...
class TestTruncation:
	def test_output_size(self):
		for _ in range(iterations):
			...
			assert len(sel.truncation(pop, outsize, **config['parent_selection_kwargs'])) == outsize
	...
	def test_no_baddies(self):
		for _ in range(iterations):
			...
			out = sel.truncation(pop, outsize, **config['parent_selection_kwargs'])
			for ind in worst_individuals:
				assert ind not in out
	...
	def test_no_duplicates(self):
		for _ in range(iterations):
			...
			outsize = random.randint(1, popsize)
			hits = {x:0 for x in pop}
			for individual in sel.truncation(pop, outsize, **config['parent_selection_kwargs']):
				hits[individual] += 1
			for hit in hits.values():
				assert hit <= 1
	...
	def test_population_unmodified(self):
		for _ in range(iterations):
			...
			copies = [copy.deepcopy(x) for x in pop]
			outsize = random.randint(1, popsize)
			selection = sel.truncation(pop, outsize, **config['parent_selection_kwargs'])
			for i in range(popsize):
				assert same_object(pop[i], copies[i])