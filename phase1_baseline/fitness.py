...
def repair_and_test_map(genotype, height, width, return_repair_count = False, agent_type='pill', samples=5, **kwargs):
	...
	for i in range(samples):
		...
		while not game.gameover:
			...
			# select uniform random valid actions for ghosts
			[game.register_action(random.choice(game.get_actions(player = player)), player = player) for player in game.players if player != 'm']
			game.step()
