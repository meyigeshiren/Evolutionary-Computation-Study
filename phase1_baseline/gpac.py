...
class GPacGame():
	def __init__(self, game_map, pill_density=0.1, fruit_prob=0.2, fruit_score=10, time_multiplier=2, num_ghosts=3, num_pacs=1):
		...
		for x in range(len(self.map)):
			for y in range(len(self.map[x])):
				...
				if self.map[x][y] == 0 and random.random() <= self.pill_density:
					self.pills.add((x,y))
		if len(self.pills) == 0: # failsafe logic to guarantee
